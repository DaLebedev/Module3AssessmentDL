...
import controller.ListItemController;
...
public class RunProgram {
	...
	static ListItemController lic = new ListItemController();
	...
	private static void editItem() {
		...
		try {
			if (searchBy == 1) {
				...
				foundItems = lic.searchMovieByTitle(movieTitle);
				
			} else if (searchBy == 2) {
				System.out.print("\nEnter the director name: ");
				String movieDirector = in.nextLine();
				foundItems = lic.searchMovieByDirector(movieDirector);
				
			} else if (searchBy == 3){
				System.out.print("\nEnter the genre: ");
				String movieGenre = in.nextLine();
				foundItems = lic.searchMovieByGenre(movieGenre);
			} else {
		...
		if ( foundItems != null && !foundItems.isEmpty()) {
			...
			ListItem toEdit = lic.searchMovieById(idToEdit);
	...
	private static void viewList() {
		...
		List<ListItem> allItems = lic.showAllMovies();
